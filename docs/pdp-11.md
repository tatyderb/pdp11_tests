...
При трассировки (простой) должны выводиться следующие данные (как в листинге + флаги `NZVC` в квадратных скобках + вовлекаемые регистры, кроме `pc`):

UPD: **Значения вовлекаемых регистров до и после операции можно не печатать**

    002346: [1100]	mov #1024, (r5)+		r5=002000
...
|-|------|-----------|----------------------|------------------------------|
|0|000rrr|`r`        |регистровый (register)|значение – содержимое регистра
|1|001rrr|`(r)` `@r` |косвенный регистровый (register deferred)|в регистре содержится адрес `op_adr = r; op_val = mem[r];`|
|2|010rrr|`(r)+ `    |автоувеличения (auto-increment)|в регистре содержится адрес, после доступа к регистру его содержимое увеличивается на 1 (если операция байтовая) или на 2. `op_adr = r; r += 2 или 1; op_val = mem[op_adr];` В случае `sp` или `pc` всегда `r += 2;`
|2|001111|`#nn`      |непосредственный (immediate)| Ассемблер помещает nn в ячейку ПОСЛЕ команды.  Используют для обозначения констант `#224`. После считывания инструкции `pc` указывает на `nn` `op_adr = pc; pc += 2; op_val = mem[op_adr]`, то есть `nn;` `pc` указывает на следующую инструкцию|
|3|011rrr|`@(r)+`    |косвенный режим автоувеличения (auto-increment deferred)| `r += 2; op_adr = mem[r]; op_val = mem[op_adr];` Значение регистра ВСЕГДА увеличивается на 2 вне зависимости от типа операции|
|3|011111|`@#nn`     |абсолютный (absolute)| берется число по указанному в `nn` адресу|
|4|100rrr|`–(r)`     |автоуменьшение (auto-decrement)|в регистре содержится адрес, еще ДО доступа к регистру его содержимое уменьшается на 1 (если операция байтовая) или на 2. `r – = 2 или 1; op_adr = r; op_val = mem[r];` В случае `sp` или `pc` всегда `r –= 2;`|
|5|101rrr|`@–(r)`    |косвенный режим автоуменьшения (auto-decrement deferred)|`r – = 2; op_adr = mem[r]; op_val = mem[op_adr];` Значение регистра ВСЕГДА уменьшается на 2 вне зависимости от типа операции|
|6|110rrr|`nn(r)`    |индексный (index)|Ассемблер кладет `nn` после инструкции.  Адресом является содержимое регистра и указанное число `nn`: считываем `nn`, при этом `pc += 2; op_adr = r; op_adr += nn; op_val = mem[op_adr];` Содержимое регистра НЕ меняется.|
...
|7|111rrr|`@nn(r)`   |косвенный индексный (index deferred)|Извлекается<br>`op_adr = r; op_adr += nn; op_adr = mem[op_adr];`<br>`op_val = mem [ap_adr];`|
